...
<div class='container sb'>
    <div class='wrapper fs'>
        <div class='left-content'>
            <ul class='breadcrumbs'>
                ...
                <?php if($product_selector){
                    if($product === false) {
                    //if it's a sub-item
                    echo "<li>". "<a href='" . DIR_ROOT . $subItem['url']."'>" . $subItem['name'] . "</a><i class='fa fa-angle-right galArrow'></i></li>";
                    echo "<li class='topMargin'>Gallery</li>";
                    } else {
                    echo "<li>". "<a href='" . DIR_ROOT . $product['selector']."'>" . $product['name'] . "</a><i class='fa fa-angle-right galArrow'></i></li>";
                    echo "<li class='topMargin'>Gallery</li>";
                    }
                }else{
                    echo "<li>Gallery</li>";
        ...
        $getGalVids = mysql_query( "SELECT * FROM product_videos WHERE selector = '" . $product_selector . "' AND gallery_video = 1" );
        $num_vids = mysql_num_rows($getGalVids);
        ...
        if($product_selector){
            ...
            //if it's a sub item
            if($product === false) {
                $path = '../_assets/_images/_products/' . $subItem['parent'] . '/' . $subItem['url'] . '/_gallery/';
                $convPath2 = '/_assets/_images/_products/' . $subItem['parent'] . '/' . $subItem['url'] . '/_gallery/';
                $thumbPath = '/_assets/_images/_products/' . $subItem['parent'] . '/' . $subItem['url'] . '/_gallery/_thumbnails/';
            }
            if( is_dir( $path ) ) {
                ...
                while( $image = readdir( $handle ) ) {
                    ...
                    if( $image != '.' && $image != '..' && $image != "Thumbs.db" && $image != "_notes" && $image != "_thumbnails") {
                        ...
                        if( $i <= $limit || $limit == 0 ) {
                            ...
                            if($product === false) {
                                $imgsrc = DIR_IMAGES . '_products/' . $subItem['parent'] . '/' . $subItem['url'] . '/_gallery/' . $image;
                                $image_dir = DIR_IMAGES . '_products/' . $subItem['parent'] . '/' . $subItem['url'] . '/_gallery/';
                            }
                            ...
                            echo "'>";
                            echo "<div class='image animate'>";
                            echo "<img alt='" . $imgalt . "' class='animate lb";
                            if( SET_LAZY_LOAD == 'true' ) {
                                echo " lazy";
                            }
                            echo "' data-group='" . $product_selector . "' data-id='" . $i . "' data-source='../serve.php?source=".$image_dir."&amp;image=".$image."&amp;thumb=1' itemprop='image' src='" . $path . '_thumbnails/' . $path_parts['filename'] . '_med' . '.' . $path_parts['extension'] ."' />";
                            echo "</div>";
                            echo "</a>";
                        } else {
