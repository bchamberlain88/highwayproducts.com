...
function titleExists($product_selector) {
	...
	if(mysql_num_rows($checkPage) > 0){
 	...
 	if($host == 'www.new.highwayproducts.com/about/') {
 		return 'About Us | Highway Products, Inc';
 	}
 	if($host == 'www.new.highwayproducts.com/contact/') {
			...
			if( $slide != "." && $slide != ".." && $slide != "Thumbs.db" ) {
				...
				if( $limit == 0 ) {
					...
					if( SET_LAZY_LOAD == 'true' ) {
						if( $i == 1 ) {
						echo "now' alt='" . $product['name'] . "' src='" . $imgsource . $imgname . "' />";
					} else {
						echo "delayLoad lazy' alt='" . $product['name'] . "' src='" . $placeholderSource . "' data-original='" . $imgsource . $imgname . "' />";
					} 
				}
				echo "</div>";
			} else {
			...
			if( $slide != '.' && $slide != '..' && $slide != "Thumbs.db" ) {
				...
				if( $limit == 0 ) {
					...
					if( $i > $limit ) {
						...
						// echo out the slide
						if( $dir == "_about" ) {
							echo "<div class='slide animate slow' data-slide='" . $i . "'><img class='lazy' alt='About Slide " . $i . "' src='../serve.php?source=".$imgdir."&amp;image=".$slide."&amp;thumb=1' /></div>";
						} else {
	...
	while( $feature = mysql_fetch_assoc( $getFeatures ) ) {
		...
		if( in_array( $selector, $parents ) ) {
			...
			if( $i <= $limit || $limit == 0 ) {
				...
				echo "' data-group='" . $selector . "-features' data-id='" . $i . "' data-source='serve.php?source=".$image_dir."&amp;image=".$image_src."&amp;thumb=1' src='".$imgThumbSrc."' />";
				echo "</a>";
				if($feature['has_video'] == 1){
		...
		if($i <= $limit){
			echo "<a class='side-gallery-img dv-3'>";
				echo "<div class='image animate'>";
					...
					echo "' data-group='" . $selector . "' data-id='" . $i . "' data-source='./serve.php?source=".$path."&amp;image=".$image."&amp;thumb=1' itemprop='image' src='" . $path . '_thumbnails/' . $path_parts['filename'] . '_sm' . '.' . $path_parts['extension'] ."' />";
				echo "</div>";
			echo "</a>";
		} else { 
		//limit reached 
...
function makeFeatureThumbs( $selector) {
	...
	while( $feature = mysql_fetch_assoc( $getFeatureInfo ) ) {
		if ($product['is_base'] == 1) {
			...
			$featDirPath = '/home/highwayp/public_html/new/_assets/_images/_products/' . $product['parent'] . '/' . $selector . '/_features/' . $feature['selector'] . '/' ;
		} else {
			$featPath = '/_assets/_images/_products/' . $parent['parent'] . '/' . $product['parent'] . '/' . $selector . '/_features/' . $feature['selector'] . '/' ;
			$featDirPath = '/home/highwayp/public_html/new/_assets/_images/_products/'. $parent['parent'] . '/' . $product['parent'] . '/' . $selector . '/_features/' . $feature['selector'] . '/' ;
		}
...
function showVideo( $source = 'product', $selector ) {
	...
	if( mysql_num_rows( $getVideos ) > 0 ) {
		...
		if( $video['type'] == 'youtube' && $video['bottom_padding'] == 1 ) {
			echo "<iframe style='border: 0;' class='player youtube bottom' height='0' src='//www.youtube.com/embed/" . $video['code'] . "?rel=0&amp;wmode=transparent' allowfullscreen></iframe>";
		}
		elseif( $video['type'] == 'youtube' ) {
			echo "<iframe style='border: 0;' class='player youtube' height='0' src='//www.youtube.com/embed/" . $video['code'] . "?rel=0&wmode=transparent' allowfullscreen></iframe>";
		}
		// embed a vimeo video player
		if( $video['type'] == 'vimeo' && $video['bottom_padding'] == 1 ) {
			echo "<iframe class='player vimeo bottom' src='//player.vimeo.com/video/" . $video['code'] . "?title=0&amp;byline=0&amp;portrait=0&amp;badge=0&amp;color=ffffff' width='100%' height='0' frameborder='0' webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>";
		}
