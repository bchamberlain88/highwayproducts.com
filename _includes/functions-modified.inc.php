				...
				if( $limit == 0 ) {
					...
					echo "<div class='slide animate slow' data-slide='" . $i . "'>";
						...
						if( SET_LAZY_LOAD == 'true' ) {
							if( $i == 1 ) {
							echo "now' src='" . $imgsource . $imgname . "' />";
						} else {
							echo "delayLoad lazy' src='" . $placeholderSource . "' data-original='" . $imgsource . $imgname . "' />";
						} }
					echo "</div>";
				} else {
					...
					if( $i > $limit ) {
						...
						echo "<div class='slide animate slow' data-slide='" . $i . "'>";
							...
							echo "' src='" . $placeholderSource . "' data-original='" . $imgsource . "' />";
						echo "</div>";
					}
				}
			}
				...
				echo "' data-feature='".$feature['selector']."' id='feature-" . $i . "'>";
					...
					echo"'>";
						echo "<a class='image-container animate'>";
							...
							echo "' data-group='" . $selector . "-features' data-id='" . $i . "' data-source='serve.php?source=".$image_dir."&image=".$image_src."&thumb=1' src='serve.php?source=".$image_dir."&image=".$image_src."&thumb=1' />";
						echo "</a>";
						if($feature['has_video'] == 1){
...
function galleryThumbs( $source = 'product', $selector, $limit = 15 ) {
	...
	foreach($list as $image){
		$i++;
		//set conversion path for thumbnails
		$convPath = '/home/highwayp/public_html/new';
		$dirCheck = $convPath . $thumbPath;
		...
		$smallThumb = '/usr/bin/convert' . ' ' . $convPath . $convPath2 . $image . ' -resize 97x68 ' . $convPath . $thumbPath . $path_parts['filename'] . '_sm' . '.' . $path_parts['extension'];
		$medThumb = '/usr/bin/convert' . ' ' . $convPath . $convPath2 . $image . ' -resize 265x199 ' . $convPath . $thumbPath . $path_parts['filename'] . '_med' . '.' . $path_parts['extension'];

		$checkHash = mysql_query( "SELECT * FROM thumbnail_compare WHERE thumbhash = '" . $currentHash . "'" );
		 ...
		 if(mysql_num_rows($checkHash)>0) {
			...
			exec($smallThumb);
			$addHash = mysql_query("INSERT INTO thumbnail_compare (thumbhash) VALUES ('$currentHash')");
		}
		if($i <= $limit){
...
function showVideo( $source = 'product', $selector ) {
	...
	if( mysql_num_rows( $getVideos ) > 0 ) {
		...
		if( $video['type'] == 'youtube' || $video['bottom_padding'] == 1 ) {
			echo "<iframe class='player youtube bottom' width='100%' height='0' src='//www.youtube.com/embed/" . $video['code'] . "?rel=0&wmode=transparent' frameborder='0' allowfullscreen></iframe>";
		}
