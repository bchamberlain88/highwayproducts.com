...
<body>
<?php include_once("../_includes/connect.inc.php"); include("./functions.php"); ?>

<form action="./" class="search-form" method="post">
    <div class="logo"><div class="logo-mask"></div></div>
    <input autocomplete="off" class="search-input" type="text" name="query" placeholder="Search by image number" value=<?php echo "'".$_POST['query']."'"; ?> />
    <!-- <input class="search-button" type="submit" name="search" value="Search For Images" /> -->
...
<?php if($_POST['search']){ $search = $_POST['query'];
$query = mysql_query("SELECT * FROM images WHERE picture_code LIKE '%$search%'");
$return = mysql_num_rows($query); if($return > 0){ while($row = mysql_fetch_assoc($query)){

echo "<div class='result'>
        <a alt='".$row['random']."' class='result-link' href='" . DIR_ROOT . $row['image_path'].$row['filename']."'>
          <div class='view'></div>
          <img src='" . DIR_ROOT . $row['image_path'].$row['filename']."' />
        </a>
      </div>";

}}else{ echo "Your search returned zero results"; }}else{
directory(); } ?>
